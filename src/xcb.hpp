...
public:
    /* intern_atom_cok_t(xcb_intern_atom_cookie_t __cookie) { this->_cookie =
     * __cookie; } */
    iAtomC(bool __only_if_exists, const char* __name)
        : _cookie(xcb_intern_atom(conn, __only_if_exists, slen(__name), __name))
    {}

    operator xcb_intern_atom_cookie_t()
    {
        return _cookie;
    }
    operator const xcb_intern_atom_cookie_t&() const
    {
        return this->_cookie;
    }
};
...
inline void setErrState(Type& first, Args&... args)
{
    ERR_STATE(first);
...
public:
    ...
    bool is_not_valid()
    {
        return (response_type == 1 << 7 && pad0 == 1 << 7 && sequence == 1 << 7 && length == 1 << 7 && atom == 1 << 7);
    }
...
public:
    wAttrC(uint32_t __w) : _cookie(xcb_get_window_attributes(conn, __w))
    {}

    operator xcb_get_window_attributes_cookie_t()
    {
        return _cookie;
    }
    operator const xcb_get_window_attributes_cookie_t&() const
    {
        return this->_cookie;
    }
};
...
public:
    ...
    wAttrR(xcb_get_window_attributes_cookie_t __cookie)
    {
        ...
        {
            set_ERR_STATE(response_type, backing_store, sequence, length, visual, _class, bit_gravity, win_gravity,
                          backing_planes, backing_pixel, save_under, map_is_installed, map_state, override_redirect,
                          colormap, all_event_masks, your_event_mask, do_not_propagate_mask, pad0[0], pad0[1]);
        }
    }

    bool is_not_valid()
    {
        return ((response_type == 1 << 7 && backing_store == 1 << 7 && sequence == 1 << 7 && length == 1 << 7 &&
                 visual == 1 << 7 && _class == 1 << 7 && bit_gravity == 1 << 7 && win_gravity == 1 << 7 &&
                 backing_planes == 1 << 7 && backing_pixel == 1 << 7 && save_under == 1 << 7 &&
                 map_is_installed == 1 << 7 && map_state == 1 << 7 && override_redirect == 1 << 7 &&
                 colormap == 1 << 7 && all_event_masks == 1 << 7 && your_event_mask == 1 << 7 &&
                 do_not_propagate_mask == 1 << 7 && pad0[0] == 1 << 7 && pad0[1] == 1 << 7));
    }
...
namespace XCB
{
    ...
    void create_window(uint32_t __window, uint32_t __parent, int16_t __x, int16_t __y, uint16_t __width,
                       uint16_t __height, uint8_t __depth = screen->root_depth, uint16_t __border_width = 0,
                       uint16_t ___class = XCB_WINDOW_CLASS_INPUT_OUTPUT, uint32_t __visual = screen->root_visual,
                       uint32_t __value_mask = 0, const void* __value_list = nullptr);
...
public:
    ...
    inline bool check_req_err()
    {
        return ((_flags & (1ULL << X_REQ_ERROR)) != 0);
    }
